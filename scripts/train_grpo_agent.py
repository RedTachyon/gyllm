"""
Train a NanoLLM model with GRPO using an Agent layer.
"""
...
from nanorl.rl import compute_grpo_advantages, compute_grpo_loss
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(description="Train a NanoLLM model with GRPO using an agent.")
    parser.add_argument(
        "--config",
        default=None,
        help="Path to a YAML config (defaults to scripts/configs/grpo_gsm8k.yaml).",
    )
    parser.add_argument(
        "--print-config",
        action="store_true",
...
def main(argv: Iterable[str] | None = None) -> None:
    """Run GRPO training with a configurable agent implementation."""
    parser = build_parser()
    args, overrides = parser.parse_known_args(list(argv) if argv is not None else None)

    config_path = Path(args.config) if args.config else Path(__file__).resolve().parent / "configs" / "grpo_gsm8k.yaml"
    ...
    with ExitStack() as stack:
        ...
        for update in trange(cfg.num_updates):
            ...
            advantages, group_metrics = compute_grpo_advantages(rollouts)
            ...
            for start in range(0, total_rollouts, cfg.minibatch_size):
                ...
                minibatch_adv = advantages[start : start + cfg.minibatch_size]
                loss, mb_metrics = compute_grpo_loss(
                    minibatch,
                    minibatch_adv,
                    model,
                    tokenizer,
            ...
            metrics = {
                "avg_reward": group_metrics["avg_reward"],
                "avg_logprob": total_logprob / total_assistant_tokens,
                "assistant_tokens": float(total_assistant_tokens),
                "adv_mean": group_metrics["adv_mean"],
                "adv_std": group_metrics["adv_std"],
                "group_count": group_metrics["group_count"],
                "group_size_mean": group_metrics["group_size_mean"],
            }
